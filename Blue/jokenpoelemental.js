...
//while para Perguntar se o usuario quer continuar uma nova sequencia de rodadas.
while(reiniciar != "n"){
  registro=[[],[],[]];pontosp1=0,pontospc=0;//Reset de pontos e registro.
  ...
  console.log();
  while(true){
  reiniciar = prompt("Deseja iniciar um novo jogo?(S/N)");
  reiniciar = reiniciar.toLowerCase();
    if(reiniciar == "n" || reiniciar == "nao" || reiniciar == "não"){
      reiniciar = "n"
      break; 
    }else if(reiniciar == "s" || reiniciar == "sim"){
      console.clear();
      break;
    }
    console.clear();
